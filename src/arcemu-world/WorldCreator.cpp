...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			...
			if(pGroup != NULL) // we are in a group
			{
				...
				if((inf->type == INSTANCE_MULTIMODE && grpdiff == MODE_HEROIC) || inf->type == INSTANCE_RAID)
				{
					...
					if(plr->GetPersistentInstanceId(mapid, grpdiff) == 0)
					{
						...
						else if( sWorld.getWorldConfig().instanceHandling.takeGroupLeaderId )
						{
	...
	if( sWorld.getWorldConfig().instanceHandling.slidingExpiration )
	{
	...
	else
	{
		if(inf->type == INSTANCE_MULTIMODE && in->m_difficulty >= MODE_HEROIC)
		{
			in->m_expiration = UNIXTIME - (UNIXTIME % TIME_DAY) + ((UNIXTIME % TIME_DAY) > (sWorld.getWorldConfig().instanceHandling.dailyHeroicResetHour * TIME_HOUR) ? 82800 : -3600) + ((sWorld.getWorldConfig().instanceHandling.dailyHeroicResetHour - sWorld.getWorldConfig().server.timezone ) * TIME_HOUR);
		}
		else if(IS_PERSISTENT_INSTANCE(in))
		{
			if(m_nextInstanceReset[in->m_mapId] == 0)
			{
				m_nextInstanceReset[in->m_mapId] = UNIXTIME - (UNIXTIME % TIME_DAY) - ((sWorld.getWorldConfig().server.timezone + 1) * TIME_HOUR) + (in->m_mapInfo->cooldown == 0 ? TIME_DAY : in->m_mapInfo->cooldown);
