...
bool StartConsoleListener()
{
	...
	return false;
#else
	std::string lhost = sWorld.getWorldConfig().remoteConsole.address;
	uint32 lport = sWorld.getWorldConfig().remoteConsole.port;
	bool enabled = sWorld.getWorldConfig().remoteConsole.enabled;
